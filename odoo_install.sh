...
if [ $IS_ENTERPRISE = "True" ]; then
    ...
    echo -e "\n---- Added Enterprise code under $OE_HOME/src/enterprise/addons ----"
...
echo -e "\n---- Create TEST OE cutom modules directory ----"
sudo su $OE_USER -c "mkdir $OE_HOME/$TEST_OE_NAME"


echo -e "\n---- Setting permissions on home folder ----"
sudo chown -R $OE_USER:$OE_USER $OE_HOME/*

# Creating Config Files For Prod And Test OE

echo -e "* Create PROD server config file"

sudo touch /etc/${OE_PROD_CONFIG}.conf
echo -e "* Creating server config file"
sudo su root -c "printf '[options] \n; This is the password that allows database operations:\n' >> /etc/${OE_PROD_CONFIG}.conf"
sudo su root -c "printf 'admin_passwd = ${OE_SUPERADMIN}\n' >> /etc/${OE_PROD_CONFIG}.conf"
sudo su root -c "printf 'xmlrpc_port = ${PROD_OE_PORT}\n' >> /etc/${OE_PROD_CONFIG}.conf"
sudo su root -c "printf 'logfile = /var/log/${OE_USER}/${OE_PROD_CONFIG}.log\n' >> /etc/${OE_PROD_CONFIG}.conf"
if [ $IS_ENTERPRISE = "True" ]; then
...
sudo touch /etc/${OE_TEST_CONFIG}.conf
echo -e "* Creating server config file"
...
echo -e "* Create PROD init file"
cat <<EOF > ~/$OE_PROD_CONFIG
#!/bin/sh
### BEGIN INIT INFO
# Provides: $OE_PROD_CONFIG
...
CONFIGFILE="/etc/${OE_PROD_CONFIG}.conf"
# pidfile
...
sudo mv ~/$OE_PROD_CONFIG /etc/init.d/$OE_PROD_CONFIG
sudo chmod 755 /etc/init.d/$OE_PROD_CONFIG
sudo chown root: /etc/init.d/$OE_PROD_CONFIG

echo -e "* Start ODOO on Startup"
sudo update-rc.d $OE_PROD_CONFIG defaults

echo -e "* Starting Prod Odoo Service"
sudo su root -c "/etc/init.d/$OE_PROD_CONFIG start"
echo "-----------------------------------------------------------"
echo "Done! The Odoo server is up and running. Specifications:"
echo "Port: $PROD_OE_PORT"
echo "User service: $OE_USER"
echo "User PostgreSQL: $OE_USER"
echo "Code location: $OE_USER"
echo "Addons folder: $OE_USER/$OE_PROD_CONFIG/"
echo "Start Odoo service: sudo service $OE_PROD_CONFIG start"
echo "Stop Odoo service: sudo service $OE_PROD_CONFIG stop"
echo "Restart Odoo service: sudo service $OE_PROD_CONFIG restart"
echo "-----------------------------------------------------------"
